...
onready var itemMenu_Button_SellItem = get_node("Panel/WindowDialog_ItemMenu/ItemMenu_Button_SellItem")
...
func _on_ItemList_item_rmb_selected(index, atpos):
	...
	itemMenu_Button_SellItem.set_text("(" + String(itemData["amount"]) + ") Sell" )
...
func _on_ItemMenu_Button_SellItem_pressed():
	var newAmount = Global_Player.inventory_sellItem(dropItemSlot)
	if (newAmount < 1):
		itemMenu.hide()
	else:
		itemMenu_Button_SellItem.set_text("(" + String(newAmount) + ") Sell")
	update_slot(dropItemSlot)
	update_slot(1)